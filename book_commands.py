...
@input_error
def exit_func():
    AddressBook().address_book_save(CONTACTS)
    ...
    while True:
        ...
        if name:
            if name in CONTACTS.data.keys() and add_contact:
                ...
                return name
            print(f"Name must not be a digit")
        print("Name is a test field. Please enter name. You didn't type anything")
    ...
    while True:
        ...
        for number in phones.split():
            if not re.fullmatch(r"\+\d{12}", number):
                print(f"Invalid phone number:{number}, enter the phone number in the format: (+380*********)")
                check = False
        if check:
            return phones.split()
